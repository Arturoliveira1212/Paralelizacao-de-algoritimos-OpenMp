/**
 * Eliminação Gaussiana (Triangulação de Matrizes) - Versão Paralela
 *
 * Este programa implementa o algoritmo de Eliminação Gaussiana para
 * transformar uma matriz em sua forma triangular superior usando OpenMP.
 *
 * Uso: ./triangulacao_paralelo <arquivo_entrada>
 *
...
/**
 * Realiza a eliminação gaussiana (triangulação) - Versão Paralela
 * Transforma a matriz em forma triangular superior usando OpenMP
 */
...
// Paraleliza o processamento das linhas abaixo do pivô
#pragma omp parallel for private(j, k, fator) shared(matriz, i, linhas, colunas)
...
#pragma omp single
    ...
    printf("=== ELIMINAÇÃO GAUSSIANA (PARALELO) ===\n");
    ...
    // Realiza a eliminação gaussiana paralela
    printf("\nRealizando eliminação gaussiana (paralelo)...\n");
    eliminacao_gaussiana_paralela(matriz, ordem, ordem);
